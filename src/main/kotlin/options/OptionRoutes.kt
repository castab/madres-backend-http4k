...
fun menuOptionRoutes(): RoutingHttpHandler {
  ...
  fun <T> handleOptions(entries: Array<T>): Response where T : Enum<T>, T : madres.backend.options.Option = entries
    .map {
      Option(it.name, it.display, it.description)
    }.toJsonResponse()

  return routes(
    "/options/entree" bind Method.GET to
      cacheFilter.then {
        handleOptions(EntreeOption.entries.toTypedArray())
      },
    "/options/beverage" bind Method.GET to
      cacheFilter.then {
        handleOptions(BeverageOption.entries.toTypedArray())
      },
    "/options/appetizer" bind Method.GET to
      cacheFilter.then {
        handleOptions(AppetizerOption.entries.toTypedArray())
      },
    "/options/modifier" bind Method.GET to
      cacheFilter.then {
