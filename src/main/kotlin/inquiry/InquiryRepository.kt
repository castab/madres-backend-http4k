...
class InquiryRepository(
  ...
  fun insertNewInquiry(newInquiry: NewInquiry): Unit = jdbi.inTransaction<Unit, Exception> { handle ->
    val exists =
      handle
        .createQuery(existsStatement)
  ...
  fun getInquiryById(inquiryId: UUID): ExistingInquiry? = jdbi.withHandle<ExistingInquiry?, Exception> { handle ->
    handle
  ...
  private val existingInquiryRowMapper =
    RowMapper { rs, _ ->
      ExistingInquiry(
